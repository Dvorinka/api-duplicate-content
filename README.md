...
git clone https://github.com/your-username/api-duplicate-content.git
cd api-duplicate-content
...
- **Base URL**: http://localhost:8080/v1/duplicate/
...
```bash
curl -H "Authorization: Bearer dev-duplicate-key" \
     http://localhost:8080/v1/duplicate/endpoint
```
...
curl -H "X-RapidAPI-Proxy-Secret: your-secret" \
     ...
     https://your-api.p.rapidapi.com/v1/duplicate/endpoint
```
...
```bash
# Build image
docker build -t api-duplicate-content .

# Run container
docker run -p 8080:8080 \
  -e DUPLICATE_CONTENT_API_KEY=your-key \
  -e ENVIRONMENT=development \
  api-duplicate-content
```
...
2. Repository: `your-username/api-duplicate-content`
...
| `DUPLICATE_CONTENT_API_KEY` | Service API key | dev-duplicate-key |
...
DUPLICATE_CONTENT_API_KEY=your-development-key
```
...
DUPLICATE_CONTENT_API_KEY=your-production-api-key
...
2. **Docker Build Issues**
   ...
   docker build --no-cache -t api-duplicate-content .
   ```

3. **Environment Issues**
   ```bash
   # Check environment variables
   docker-compose logs duplicate-content-api
   ```
...
### Base URL
```
http://localhost:8080/v1/duplicate/
```
